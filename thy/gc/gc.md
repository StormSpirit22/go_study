...
这种方式的回收精度低，一个对象即使被删除了最后一个指向它的指针也依旧可以活过这一轮，在下一轮GC中被清理掉。删除写屏障也会被用在栈上。
...
删除写屏障就是将**被删除**的对象，如果为白色或灰色则赋值成灰色，这样即使再有黑色对象引用它的下游对象，也能满足至少有一条从灰色到白色的路径，满足弱三色不变性。比如 A(灰) -> B(白) -> C(白)，删除 A 对 B 的引用，那么 B 就变成灰色了，假设然后 D(黑) 引用 C，此时也满足了弱三色不变性。而如果没有对象再引用它(B)，则需要在下一轮回收过程中将它回收（回收精度低）。
...
1. GC 开始将栈上**所有可达对象**全部扫描并标记为黑色（剩下不可达的都是白色最后就直接回收，不需要第二次重复扫描，无需 STW）。
...
混合写屏障消除了栈的重扫过程，一旦栈被扫描变为黑色，则它会继续保持黑色， **并要求将创建的新对象也分配为黑色**。
