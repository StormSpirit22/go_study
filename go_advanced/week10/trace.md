...
- 固定采样，1/1024：
  ...
  这个简单的方案对我们的高吞吐量的线上服务来说是非常有用，因为那些感兴趣的事件（在大吞吐量的情况下）仍然很有可能经常出现，并且通常足以被捕捉到。然而，在较低的采样率和较低的传输负载下可能会导致错过重要事件，而想用较高的采样率就需要能接受的性能损耗。对于这样的系统的解决方案就是覆盖默认的采样率，这需要手动干预的，这种情况是我们试图避免在 Dapper 中出现的。
