...
如果想要用户可以自定义一些配置，我们可以看看 grpc 的配置定义，主要的思路就是把 option 改成接口，然后定义了一个 `EmptyCallOption` **实现这个接口**，因为这个接口包含的函数是未导出的，所以我们只要在需要做配置的 struct 当中包含这个 `EmptyCallOption` 就可以了。
