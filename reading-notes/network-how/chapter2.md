...
更多粘包拆包可参考 [面试题：聊聊TCP的粘包、拆包以及解决方案](https://cloud.tencent.com/developer/article/1804413)
...
![断开连接的交互过程](/Users/tianyou/Documents/Github/go_study/.go_study/assets/network-how/2.4-1.png)主动关闭连接方在 TCP 四次挥手中发送完最后一次 ACK 信号结束后，开始进入 time-wait 状态，而不会马上删除套接字。time-wait 的持续时间为2MSL. MSL是Maximum Segment Lifetime, 译为“报文最大生存时间”。

为什么要持续这么长的时间呢？

以客户端主动断开为例：

原因 1：为了保证客户端发送的最后一个ack报文段能够到达服务器。因为这最后一个 ack 确认包可能会丢失，然后服务器就会超时重传第三次挥手的 fin 信息报，客户端再重传一次第四次挥手的 ack 报文。如果没有这 2msl，客户端发送完最后一个 ack 数据报后直接关闭连接，那么就接收不到服务器超时重传的fin信息报(**此处应该是客户端收到一个非法的报文段，而返回一个 RST 的数据报，表明拒绝此次通信，然后双方就产生异常，而不是收不到。**)，那么服务器就不能按正常步骤进入 close 状态。那么就会耗费服务器的资源。当网络中存在大量的 timewait 状态，那么服务器的压力可想而知。

原因 2：在第四次挥手后，经过 2msl 的时间足以让本次连接产生的所有报文段都从网络中消失，这样下一次新的连接中就肯定不会出现旧连接的报文段了。如果没有这个等待，连接断开后，这个端口恰好被新的程序使用，然后客户端重发的 FIN 此时刚好到达（最后一次 ACK 没有收到），那么新的程序接收到 FIN 就会开始执行断开连接操作了。

### 小结

![TCP的总体流程](/Users/tianyou/Documents/Github/go_study/.go_study/assets/network-how/2.4-2.png)
